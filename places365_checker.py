...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_dir', type=str, default='/Users/jose.mena/dev/personal/data/Places365-Standard',
                        help='directory with the places365 dataset')
    ...
    val_data_file = 'places365_val.txt'
    val_data_file = os.path.sep.join([input_dir, val_data_file])

    val_data_df = pd.read_csv(val_data_file, sep=' ', names=['id', 'label'])
    val_data_df['class'] = val_data_df['label'].astype(str)
    val_data_df['num_index'] = val_data_df.id.str[-12:-4].astype(int)
    val_data_df = val_data_df[val_data_df.num_index > 5000]
    val_dir = os.path.sep.join([input_dir, 'val_large'])
    datagen = ImageDataGenerator()
    val_generator = datagen.flow_from_dataframe(dataframe=val_data_df, directory=val_dir, x_col="id", y_col="class",
                                                  class_mode="categorical", target_size=(224, 224), batch_size=batch_size)

    bb_model = VGG16_Places365(weights='places')
    bb_model.trainable = False

    bb_model.compile(optimizer='SGD', loss='categorical_crossentropy', metrics=['acc'])

    transfer_scores = bb_model.evaluate_generator(val_generator, steps=len(val_data_df) // batch_size, verbose=1)
